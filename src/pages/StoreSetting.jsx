    ...
    <LayoutDashboard>
      ...
      <form>
        <div className=" bg-white px-8 py-8 lg:px-20 w-full rounded-lg shadow-lg mt-6 flex flex-col md:flex-row gap-4 md:gap-6 items-center md:items-start">
          <img src={showImage ? showImage : dataStore.image} alt="image" className="w-40 h-40 rounded-full" />
          <div className="md:w-4/12 px-12 md:px-1">
            <p className="text-sm mb-4 ">Ukuran optimal 300 x 300 piksel dengan Besar file: Maksimum 10.000.000 bytes (10 Megabytes). Ekstensi file yang diperbolehkan: JPG, JPEG, PNG</p>
            <label htmlFor="store-image" className="border-2 p-2 w-56 rounded font-medium text-gray-500 border-gray-300">
              Edit Foto Toko
            </label>
            <input type="file" name="image" id="store-image" className="hidden" onChange={handleImageUpload} />
          </div>
          <div className=" md:w-4/12 px-12 md:px-1">
            ...
            <textarea rows="4" className="w-full border border-gray-300 mb-3 p-2" onChange={(e) => setDescription(e.target.value)} name="description" value={description}></textarea>
            <button className="bg-emerald-500 text-white rounded-md  font-medium p-3 w-56 flex justify-center" onClick={submitDescription} type="submit">
