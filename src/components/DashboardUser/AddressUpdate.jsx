      ...
      <div className=" w-full h-[500px] overflow-scroll lg:w-[80%] bg-white py-10 px-10 rounded-lg z-[999]">
        <div className="flex flex-col w-full justify-center items-center ">
          <div className="flex w-full items-center justify-between pb-10 border-b-2 ">
            <h2 className="mt-10 md:mt-0"> {newAddressForm === "address-form" ? "Form Alamat Baru" : newAddressForm === "address-edit" ? "Form Edit Alamat" : "List Alamat"}</h2>
            <div className="flex gap-5">
              {newAddressForm === "address-list" ? (
                ...
                <button onClick={() => OpenForm("address-list")} className="bg-green-500 py-2 px-5 rounded font-semibold">
                  Batal
                </button>
              )}
          ...
          <div className="w-full relative">
            {/* edit address */}
            {newAddressForm === "address-edit" ? <AddressEdit id="address-edit" dataEdit={addressData} setnewAddressForm={setnewAddressForm} setMessage={setMessage} setIsSuccess={setIsSuccess} getUserAddress={getUserAddress} /> : ""}
            {/* add new address */}
            {newAddressForm === "address-form" ? <AddressAdd id="address-form" setnewAddressForm={setnewAddressForm} setMessage={setMessage} setIsSuccess={setIsSuccess} getUserAddress={getUserAddress} /> : ""}
            {/* List of All Address = this is the first rendered when address list opened */}
            {newAddressForm === "address-list" ? (
              <AddressList id="addresslist" sortedAddress={sortedAddress} showConfirmationBox={showConfirmationBox} setMainAddress={setMainAddress} getUserAddress={getUserAddress} OpenForm={OpenForm} getEditFormData={getEditFormData} />
            ) : (
              ""
            )}
          </div>
